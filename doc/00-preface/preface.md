...
- **Not a distributed system on its own.** The engine stores data on one node, and replication
  comes from the layers above it. Within those layers TideSQL is a first-class participant rather
  than a generic pass-through: it carries the binlog capabilities replication needs and commits
  through the two-phase protocol that keeps a replica crash-safe. See
...
covers monitoring, backup, maintenance, and online DDL.
...
TidesDB data files live in a sibling directory of the server's data directory named
