...
The separation is meaningful. Because each column family is its own LSM B+tree, a secondary index has
...
Physically adjacent keys in the LSM B+tree are logically adjacent rows, so range scans on the primary
key are efficient.
...
bitmap and each non-null field serialized with the server's `Field::pack()`. Fixed-size fields like
...
at commit catch a concurrent duplicate. Either way the engine recovers the primary key and performs a point
...
The optimizer is aware of these indexes. The engine reports cost from the LSM B+tree's read
...
point lookup, by decoding those columns into the record buffer and calling the server's
...
The engine declines MRR and falls back to the base handler in two cases: single-range scans where
sorting wins nothing, and true range scans such as `BETWEEN`/`<`/`>`. It also declines for a
partitioned table, whose own handler dispatches the ranges across children, though that cannot
arise here — see [Partitioning](/administration/partitioning).

## Auto-increment

Auto-increment follows the server's built-in `update_auto_increment()` during `write_row()`. Rather
