...
`ANALYZE TABLE ... UPDATE HISTOGRAM` builds a histogram for a column with no index on it, which is
how the optimizer estimates selectivity for a predicate no index covers:

```sql
ANALYZE TABLE orders UPDATE HISTOGRAM ON amount WITH 16 BUCKETS;
```

```
Table        Op         Msg_type  Msg_text
shop.orders  histogram  status    Histogram statistics created for column 'amount'.
```

The histogram belongs to the server, not the engine, and is read back through
`information_schema.COLUMN_STATISTICS`. The engine's part is supplying the rows: on a table too
large for `histogram_generation_max_mem_size` the server asks for a fraction of them, and the engine
picks that fraction while scanning keys, so a row the sample does not want is never decoded. See
[Statistics and the Optimizer](/internals/optimizer) for how the fraction is chosen.

One ordering note. The fraction is the memory budget divided by the engine's row estimate, and that
estimate is cached for up to two seconds. Immediately after a bulk load it can still be low, which
makes the server sample more of the table than it intended. A plain `ANALYZE TABLE` first forces the
estimate current:
...
Because a sampled histogram is built from part of the column, its outermost buckets need not reach
the column's true minimum and maximum. Every bound in it is a real value from the table;
`JSON_EXTRACT(HISTOGRAM, '$."sampling-rate"')` reports what fraction it was built from.
