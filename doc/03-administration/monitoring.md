...
- **Write Amplification** - user bytes written, flush and compaction bytes with their SSTable
  counts, compaction bytes read, value-log bytes written, and the derived total WA ratio. The ratio
  counts value-log appends, which flush and compaction never see, so a store with value separation
  active would otherwise under-report.
...
With no transaction open there is no floor, and the counter reads `18446744073709551615`
(`UINT64_MAX`) rather than zero. That is the idle value, not a pinned snapshot in the far future, so
an agent graphing `global_sequence - min_snapshot_sequence` has to treat it as a sentinel or the
idle case swamps the graph. Alert on the gap only while `tidesdb_active_transactions` is non-zero.
