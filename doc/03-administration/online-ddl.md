...
These complete without rebuilding data. The server rewrites the table metadata and the change takes
effect immediately:
...
ALTER TABLE events ALTER COLUMN kind SET DEFAULT 'none', ALGORITHM=INSTANT;
...
-- an add and a drop in one statement
ALTER TABLE events ADD INDEX idx_ts (ts), ALGORITHM=INPLACE;
...
`HA_ALTER_INPLACE_NOT_SUPPORTED` for these with a reason, so the server falls back to `ALGORITHM=COPY`,
