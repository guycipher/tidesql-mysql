...
`FOREIGN KEY` clause is checked on every INSERT, UPDATE, and DELETE. The constraint itself is kept
in the server's data dictionary, so it survives a restart and shows up in `SHOW CREATE TABLE` and
the information schema the same way InnoDB's does; the engine reads it from there and does the
checking. The handler advertises the capability to the server, so the optimizer and the replication
layer treat these tables as referentially constrained.

Declaring foreign-key support obliges the engine to support atomic DDL as well — the server couples
the two, and takes a foreign-key branch during `DROP TABLE` that is only safe when schema changes
are transactional. The engine claims both together, and defers a column-family create or drop to
the post-DDL hook so that a rolled-back dictionary transaction leaves no storage behind.
...
A foreign key may reference a unique key, not only the primary key:
...
The referenced unique key may be nullable. A nullable column is stored with a leading indicator byte
in front of its value, so a key with one is not the same bytes as a key without, and the engine
records which of the parent's columns carry one when the constraint is created. The child's lookup
then rebuilds the parent's exact key. The two sides need not agree: a `NOT NULL` referencing column
may reference a nullable unique key, and a nullable one may reference a `NOT NULL` key.

A parent row whose unique column is NULL is not a value anything can reference — NULL does not equal
NULL — so a child row still has to match a parent row that holds a real value.
...
with descending order, because the engine matches child rows against a forward sort key that a
descending column would not line up with. It is reported as an unsupported constraint at create time
