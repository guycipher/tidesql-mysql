...
Greek. Text is split on word boundaries using the server's charset classification and lowercased with
...
family. Pointing the variable at a table on another engine, or at one that does not exist, leaves the
previous list in place and logs a warning to the error log:

```
[TIDESDB] Failed to load stop words from 'mydb/other', keeping current set
```

The `SET` itself succeeds and the variable reads back as the name you set, so reading
`@@GLOBAL.tidesdb_ft_stopword_table` does not tell you whether the list loaded — the error log is
what says so. Setting it to NULL or empty restores the default:
