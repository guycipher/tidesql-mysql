...
on the LSM B+tree. Rather than an R-tree, which needs in-place node updates that clash with
...
CREATE TABLE places (
  ...
  location GEOMETRY NOT NULL SRID 0, SPATIAL INDEX (location)
) ENGINE=TIDESDB;

INSERT INTO places VALUES (1, 'NYC', ST_GeomFromText('POINT(40.7128 -74.0060)', 0));
```

The column needs an explicit SRID. A `GEOMETRY` column without one still takes a `SPATIAL INDEX`,
but the server warns that the optimizer will not use it, and every spatial query then falls back to a
full scan no matter what the engine offers:

```
Warning 3674  The spatial index on column 'location' will not be used by the query optimizer
              since the column does not have an SRID attribute.
```

`SRID 0` is the flat Cartesian system, which is what the engine's bounding-box arithmetic assumes.

## Queries

MBR-based predicates work through the standard spatial functions:

```sql
SELECT name FROM places
WHERE MBRIntersects(location,
                    ST_GeomFromText('POLYGON((39 -76, 43 -76, 43 -72, 39 -72, 39 -76))', 0));

SELECT name FROM places
WHERE MBRWithin(location,
                ST_GeomFromText('POLYGON((25 -125, 45 -125, 45 -70, 25 -70, 25 -125))', 0));
```
...
in the LSM B+tree and read sequentially.
