#!/usr/bin/env python3
"""Typeset the TideSQL manual (doc/manual.json + its markdown) as a printed book.
...
stays in step with the site build. The title page takes its version from a
"version" key in manual.json, from the project(... VERSION) of a CMakeLists.txt
above doc/, or from --manual-version, in that order.
...
carry a reader from one place to another, as in a printed book.
    ...
    ./tools/build_manual_pdf.py                       # -> doc/tidesql-manual.pdf
...
def project_version(doc_dir: Path, manual: dict) -> str | None:
    """The version for the title page: whatever manual.json states, else the
    project(... VERSION x.y.z) of the CMakeLists.txt above doc/."""
    stated = manual.get("version")
    if stated:
        return str(stated)
    versioning = doc_dir.parent / "VERSIONING.md"
    if versioning.is_file():
        # VERSIONING.md states the pairing: "TideSQL 2.0.0 pairs with TidesDB v10.0.0"
...
def fit_code(chapters: list[dict], measure_pt: float, indent_pt: float, bleed_pt: float) -> None:
    """Size each verbatim block so its longest line fits.

    A listing is set at the base size when it fits the measure. When it does not
    it shrinks, and past a readable floor it is allowed to hang into the outer
    margin rather than shrink further -- what a printed manual does with a wide
    listing. Nothing is ever clipped."""
    room = measure_pt - indent_pt
    available = room + bleed_pt
    base = 8.4
    ...
    def size_for(block: str) -> str:
        ...
        if widest == 0:
            return block
        size = min(base, available / (widest * MONO_ADVANCE) * 0.99)
        style = []
        if size < base - 0.05:
            style.append(f"font-size:{size:.2f}pt")
        overhang = widest * MONO_ADVANCE * size - room
        if overhang > 1:
            style.append(f"margin-right:-{overhang / 72 * 25.4:.1f}mm")
        if not style:
            return block
        return block.replace("<pre", f'<pre style="{";".join(style)}"', 1)
...
def build_edition_html(manual: dict, args, version: str | None) -> str:
    """A short edition notice: which SQL server this manual is written against,
    which releases of it the plugin supports, and which library it links.

    TideSQL builds against more than one server from the same sources. Saying
    on its own page which one an edition was built for keeps a reader from
    applying instructions to a build this manual never describes."""
    host, releases = args.host, args.host_versions
    library = manual.get("tidesdb")
    rows = ["<tr><th>TideSQL</th>" + f"<th>{host}</th>" +
            ("<th>TidesDB library</th>" if library else "") + "</tr>"]
    rows.append(f"<tr><td>{version or '&#8212;'}</td><td>{releases}</td>"
                + (f"<td>{library}</td>" if library else "") + "</tr>")
    other = args.other_host
    note = (f"<p>The engine also builds against {other}, from a separate repository. "
            f"That build is not described here.</p>" if other else "")
    return f'''<section class="frontmatter edition-note"><h1>This Edition</h1>
<p>TideSQL is the storage engine that embeds the TidesDB library in MySQL-compatible servers.
This is the {host} edition of the manual: every statement, variable and operational instruction
in it is written against {host}.</p>
<table><thead>{rows[0]}</thead><tbody>{rows[1]}</tbody></table>
<p>A TideSQL release supports the {host} releases named above. It links exactly one release of
the TidesDB library and writes that library&#8217;s on-disk format.</p>
{note}
</section>'''
...
.title-page .edition {{ margin-top: auto; padding-bottom: 22mm; text-align: center;
...
.title-page h1, .frontmatter h1 {{ padding-left: .1em; }}   /* re-centre letterspaced caps */
.title-page p {{ text-indent: 0; }}   /* p + p indents running text, not a title page */
.title-page .edition {{ padding-left: .1em; }}
.title-page h1 {{
  font-size: 2.35rem; font-weight: 400; letter-spacing: .1em; text-transform: uppercase;
  margin: 0; padding: .45em 0 .45em .1em; border-top: 1.1pt solid #000; border-bottom: 1.1pt solid #000;
}}
...
.edition-note {{ text-align: left; }}
.edition-note p {{ margin: 0 0 .8em; text-indent: 0; }}
.edition-note table {{ width: auto; min-width: 60%; margin: 1.6em 0 1.8em; }}
.edition-note td, .edition-note th {{ padding-right: 2.2em; }}
...
def build_html(manual: dict, chapters: list[dict], args, version: str | None) -> str:
    ...
    subtitle = (args.subtitle if args.subtitle is not None
                else manual.get("subtitle") or "A manual for the SQL storage engine")
    edition = [f"Version {version}" if version else None,
               f"{args.host} edition" if args.host else None,
...
{build_edition_html(manual, args, version) if args.edition_page else ""}
...
def main() -> int:
    ...
    ap.add_argument("--subtitle", help="line under the title (default: a manual for the SQL storage engine)")
    ap.add_argument("--host", default="MySQL", help="the SQL server this edition targets (default: MySQL)")
    ap.add_argument("--host-versions", default="9.7",
                    help="the host releases this TideSQL release supports (default: 9.7)")
    ap.add_argument("--other-host", default="MySQL",
                    help="a host built from another repository, named but not documented (default: MySQL)")
    ap.add_argument("--no-edition-page", dest="edition_page", action="store_false",
                    help="leave out the edition notice after the title page")
    ...
    slug = re.sub(r"[^a-z0-9]+", "-", manual.get("title", "manual").lower()).strip("-")
    out = (args.output or doc_dir / f"{slug}.pdf").expanduser().resolve()
    ...
    fit_code(chapters, measure_pt, 1.5 * args.body_size, max(side - 8.0, 0) * 72 / 25.4)
    ...
    version = args.manual_version or project_version(doc_dir, manual)
    for w in warnings:
        log(f"warning: {w}")
