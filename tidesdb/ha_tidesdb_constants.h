...
/* Mirror constants for the library's TDB_DEFAULT_* values defined in
   ...
   leaks a `realloc` macro that conflicts with the server's String::realloc()
...
/* The table's storage options, persisted in the same way and for the same reason: what a table is
   stored as has to be settled when it is created.  Where the server records the options itself
   this key is never written; where it does not, an option the table did not name would otherwise
   be resolved again from whatever the session default happened to be at the next open, so the same
   table would be stored differently depending on who opened it first.  Key is
   [KEY_NS_META]["OPTS"]; value is the option set as the JSON object src/core/table_options reads.
   Living in the table's own column family, it is dropped, renamed and truncated with the table and
...
/* scan_time() -- split the sorted-run overlap count reported by
   tidesdb_range_stats between the server's I/O and CPU cost buckets.  LSM scans
...
/* server packed-field widths used by sort-key decoders. */
...
/* Separator that joins db and table names when forming a TidesDB CF name
   from a server path (e.g. "test/foo" -> "test__foo"). */
static constexpr const char CF_DB_TABLE_SEP[] = "__";

/* server temp-table marker character.  Internal temp/exchange tables
   carry one or more '#' in their on-disk name (e.g. "#sql-..."); we
   substitute '_' so the resulting CF name remains valid. */
static constexpr char TDB_TEMP_NAME_MARKER = '#';
static constexpr char TDB_TEMP_NAME_REPLACEMENT = '_';

/* Relative-path prefix that the server prepends to table paths handed
   to handler callbacks ("./db/table").  path_to_cf_name strips it
   before extracting db/table components. */
static constexpr const char TDB_REL_PATH_PREFIX[] = "./";
static constexpr size_t TDB_REL_PATH_PREFIX_LEN = 2;

/* server sort-key null-indicator bytes prepended to nullable key parts
...
/* Minimum number of point ranges in a multi-range request before our
   custom MRR path takes over from the server's default implementation.
...
/* Highest ENCRYPTION_KEY_ID a table may name.  Mirrors the upper bound in the ENCRYPTION_KEY_ID
   HA_TOPTION_NUMBER declaration, and bounds the sweep a master-key rotation makes over the stored
   table keys, so that loop terminates on a statically known limit rather than on the contents of
   the key store. */
static constexpr uint TIDESDB_MAX_ENCRYPTION_KEY_ID = 255;

/* Prefix marking a column family the engine owns for its own bookkeeping rather than one backing a
   user table.  A user table's family is built as <db>__<table> from a path the server hands the
   handler, and a database component never arrives empty, so a leading separator cannot collide with
   one.  Reserved families are excluded from the column-family listing SHOW ENGINE STATUS prints,
   which reports what the user's tables are doing. */
static constexpr const char TIDESQL_RESERVED_CF_PREFIX[] = "__";
static constexpr size_t TIDESQL_RESERVED_CF_PREFIX_LEN = 2;

/**
 * tidesql_is_reserved_cf
 * whether a column family is engine-internal rather than a user table's
 * @param name the column family name
 * @return true when the engine owns it for its own bookkeeping
 */
static inline bool tidesql_is_reserved_cf(const char *name)
{
    return name != nullptr &&
           strncmp(name, TIDESQL_RESERVED_CF_PREFIX, TIDESQL_RESERVED_CF_PREFIX_LEN) == 0;
}

/* Highest table-key version a single ENCRYPTION_KEY_ID may reach.  A version is minted only by an
   explicit key rotation, so this bound is far above any realistic key history; it exists to give
   the rotation sweep a statically verifiable upper bound rather than one read back from the key
   store, whose contents the sweep is in the middle of rewriting. */
static constexpr uint TIDESDB_MAX_ENCRYPTION_KEY_VERSIONS = 4096;
