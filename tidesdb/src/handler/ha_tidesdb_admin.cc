...
/**
 * tdb_meta_key_count
 * how many of a data column family's keys are the engine's own rather than rows
 * @param cf the table's data column family
 * @return the count, or 0 when it cannot be read
 *
 * The engine keeps a little state alongside a table's rows, in the same family and under a leading
 * namespace byte that sorts before every row: the options the table was created with, an
 * auto-increment start value, a per-index full-text document count.  The library counts those keys
 * like any others, so they are counted here too and taken back off the row estimate.  There are a
 * handful of them at most, and this runs only when the cached statistics have gone stale.
 */
static uint64_t tdb_meta_key_count(tidesdb_column_family_t *cf)
{
    ...
    /* The namespace byte alone is the lowest key the meta namespace can hold, and the next
       namespace's byte is the first key past it. */
    const uint8_t lower[] = {KEY_NS_META};
    const uint8_t upper[] = {KEY_NS_DATA};

    tidesdb_iter_t *iter = NULL;
    uint64_t count = 0;
    if (tidesdb_iter_new_range(txn, cf, lower, sizeof(lower), upper, sizeof(upper), &iter) ==
            TDB_SUCCESS &&
        iter)
    {
        ...
        while (tidesdb_iter_valid(iter))
        {
            ...
            size_t klen = 0;
            tdb_owned_buf kg(k);
            /* The range tells the iterator which sstables to open; it does not stop it, so the
               walk ends where the namespace does. */
            if (tidesdb_iter_key(iter, &k, &klen) != TDB_SUCCESS || !k || klen == 0 ||
                k[0] != KEY_NS_META)
                break;
            count++;
...
void ha_tidesdb::info_refresh_cf_stats()
{
    ...
    /* The engine's own keys live in the same family and are counted with the rows; a table with
       two rows that reported four would have the optimizer plan against a table twice its size. */
    const uint64_t meta = tdb_meta_key_count(share->cf);
    rows = rows > meta ? rows - meta : 0;
...
void ha_tidesdb::info_fill_rec_per_key()
{
    for (uint i = 0; i < table->s->keys; i++)
    {
        ...
        for (uint j = 0; j < TDB_KEY_EXT_PARTS(key); j++)
        {
            if (is_pk || is_unique)
            {
...
/*
  Remember which index a duplicate-key error was raised on.

  The server members are set here as well as the engine's own copy, because one server reads the
  value the write recorded and another clears it and asks info(HA_STATUS_ERRKEY) for it afterwards.
  Keeping a copy is what lets info() answer that second question truthfully.
*/
void ha_tidesdb::note_dup_key(uint key_no)
{
    dup_key_no_ = key_no;
    TDB_SET_DUP_KEY(this, key_no);
}

/*
  Fill in the create options the server reports for the table.

  The server asks before rendering SHOW CREATE TABLE and before a dump writes the table back out.
  The only one the engine owns is the auto-increment counter, and a dump that omits it restarts the
  sequence at 1 on reload, handing out keys the original table had already used.
*/
void ha_tidesdb::update_create_info(HA_CREATE_INFO *create_info)
{
    /* A statement that named AUTO_INCREMENT itself has already said what it wants. */
    if (create_info->used_fields & HA_CREATE_USED_AUTO) return;
    if (!table || !table->found_next_number_field) return;
...
int ha_tidesdb::info(uint flag)
{
    ...
    /* Which index a duplicate landed on.  Asked for after a duplicate-key error, by a server that
       clears its own record of it first; answering from the engine's copy is what lets the error
       name the entry and the index instead of falling back to a generic message. */
    if (flag & HA_STATUS_ERRKEY) errkey = dup_key_no_;
...
void ha_tidesdb::analyze_report_cf_stats(THD *thd, const tidesdb_cf_stats_t &st)
{
    ...
    push_warning_printf(thd, TDB_WARN_LEVEL_NOTE, ER_UNKNOWN_ERROR,
    ...
    for (uint i = 0; i < table->s->keys; i++)
    {
        ...
        if (verbose)
            push_warning_printf(thd, TDB_WARN_LEVEL_NOTE, ER_UNKNOWN_ERROR,
                                ...
                                TDB_KEY_NAME(ki), (unsigned long long)sampled,
...
int ha_tidesdb::analyze(THD *thd, HA_CHECK_OPT *check_opt [[maybe_unused]])
{
...
*/
int ha_tidesdb::optimize(THD *thd, HA_CHECK_OPT *check_opt [[maybe_unused]])
{
    ...
    if (any_locked)
    {
        push_warning_printf(thd, TDB_WARN_LEVEL_NOTE, HA_ADMIN_TRY_ALTER,
...
int ha_tidesdb::check(THD *thd [[maybe_unused]], HA_CHECK_OPT *check_opt [[maybe_unused]])
{
...
int ha_tidesdb::repair(THD *thd [[maybe_unused]], HA_CHECK_OPT *check_opt [[maybe_unused]])
{
