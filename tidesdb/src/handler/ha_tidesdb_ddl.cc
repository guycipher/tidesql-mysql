                                             ...
                                             Alter_inplace_info *ha_alter_info)
{
    ...
    static const TDB_ALTER_FLAGS_T TIDESDB_INPLACE_INDEX = TDB_ALTER_INDEX_SET;
...
bool ha_tidesdb::prepare_inplace_alter_table(TABLE *altered_table,
                                             Alter_inplace_info *ha_alter_info TDB_DD_ALTER_ARG)
{
    ...
    /* An ALTER may restate the table's storage options, and an option this build does not
       recognise is refused here for the same reason CREATE refuses it. */
    std::string opt_error;
    if (!TDB_TABLE_OPTIONS_ERROR(altered_table, &opt_error))
    {
    ...
    if (ha_alter_info->index_add_count > 0)
    {
        for (uint a = 0; a < ha_alter_info->index_add_count; a++)
        {
            ...
            std::string idx_cf = base_cf + CF_INDEX_INFIX + TDB_KEY_NAME(new_key);
                                         ...
                                         bool &row_has_null)
{
    ...
    for (uint p = 0; p < ki->user_defined_key_parts; p++)
    {
        ...
        if (TDB_FIELD_IS_NULLABLE(field))
        {
            if (field->is_null())
            {
        ...
        TDB_FIELD_SORT_STRING(field, ik + pos, kp->length);
        field->move_field_offset(-ptdiff);
        pos += kp->length;
        if (desc) invert_key_part(ik + part_start, pos - part_start);
    }
...
bool ha_tidesdb::inplace_alter_table(TABLE *altered_table,
                                     Alter_inplace_info *ha_alter_info TDB_DD_ALTER_ARG)
{
    ...
    TDB_COLUMN_MAP_SAVED old_map = TDB_USE_ALL_COLUMNS(altered_table, read_set);
    ...
    if (rc != TDB_SUCCESS || !txn)
    {
        ...
        TDB_RESTORE_COLUMN_MAP(altered_table, read_set, old_map);
        DBUG_RETURN(true);
    }
                                            ...
                                            bool commit TDB_DD_ALTER_ARG)
{
    ...
    if (ha_alter_info->handler_flags & TDB_ALTER_CHANGE_CREATE_OPTION)
...
void ha_tidesdb::commit_rebuild_index_meta(TABLE *altered_table)
{
    ...
    for (uint i = 0; i < altered_table->s->keys; i++)
    {
        ...
        tidesdb_column_family_t *icf = resolve_idx_cf(
            tdb_global, share->cf_name, TDB_KEY_NAME(&altered_table->key_info[i]), idx_name);
...
void ha_tidesdb::commit_apply_runtime_config(TABLE *altered_table [[maybe_unused]],
                                             Alter_inplace_info *ha_alter_info)
{
    ...
    if (a_opts)
    {
        share->options = *a_opts;

        uint iso_idx = a_opts->isolation_level;
        if (iso_idx < array_elements(tdb_isolation_map))
            share->isolation_level = (tidesdb_isolation_level_t)tdb_isolation_map[iso_idx];
        share->default_ttl = a_opts->ttl;
        share->has_ttl = (share->default_ttl > 0 || share->ttl_field_idx >= 0);
        share->encrypted = a_opts->encrypted;
        if (share->encrypted) share->encryption_key_id = (uint)a_opts->encryption_key_id;

        /* The ALTER settles the table's storage afresh, so what is recorded has to say so. */
        TDB_TABLE_OPTIONS_STORE(share->cf, a_opts);
    }
}

/*
  Tell the server whether changing table options requires a rebuild.
