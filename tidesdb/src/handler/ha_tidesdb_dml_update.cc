                                    ...
                                    uint new_pk_len, bool pk_changed)
{
    ...
    for (uint i = 0; i < table->s->keys; i++)
    {
        ...
        for (uint p = 0; p < ki->user_defined_key_parts; p++)
        {
            ...
            if (TDB_FIELD_IS_NULLABLE(f) && f->is_real_null(nd_ptrdiff))
            {
                any_null = true;
                break;
            }
                                 ...
                                 uint new_pk_len, bool pk_changed, time_t row_ttl)
{
    ...
    const CHARSET_INFO *fts_cs = ki->key_part[0].field->charset();
...
int ha_tidesdb::update_row(const uchar *old_data, TDB_ROW_IN *new_data)
{
