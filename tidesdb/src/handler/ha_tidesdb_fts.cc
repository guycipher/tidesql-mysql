...
/* adapter exposing the server's charset behaviour to the server-free fts core. */
struct fts_server_charset : tidesdb::fts::charset
{
    const CHARSET_INFO *cs;
    explicit fts_server_charset(const CHARSET_INFO *c) : cs(c)
    {
    }
...
void fts_tokenize(const char *text, size_t text_len, const CHARSET_INFO *cs,
                  std::vector<std::string> &out)
{
    fts_server_charset mcs(cs);
...
void fts_extract_and_tokenize(TABLE *table, const KEY *key_info, const uchar *record,
                              const CHARSET_INFO *cs, std::vector<std::string> &out_tokens)
{
    std::string doc;
...
void fts_parse_boolean(const char *query, size_t len, const CHARSET_INFO *cs,
