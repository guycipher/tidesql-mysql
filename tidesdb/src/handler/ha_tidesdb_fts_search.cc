...
{
    struct _ft_vft *please;                /* required by the FT_INFO layout    */
...
static void ft_parse_query(uint flags, const char *keyp, size_t keylen, const CHARSET_INFO *cs,
...
static bool ft_doc_matches_phrases(TABLE *table, KEY *ki, const CHARSET_INFO *vcs,
...
FT_INFO *ha_tidesdb::ft_init_ext(uint flags, uint inx, String *key)
{
    ...
    const CHARSET_INFO *cs = table->key_info[inx].key_part[0].field->charset();
    ...
    {
        const CHARSET_INFO *vcs = table->key_info[inx].key_part[0].field->charset();
