                               ...
                               enum ha_rkey_function find_flag)
{
    ...
    uint key_len = TDB_CALC_KEY_LEN(table, active_index, key, keypart_map);
                                     ...
                                     enum ha_rkey_function find_flag)
{
    ...
    for (;;)
    {
        ...
        tdb_icp_result_t icp = icp_check_secondary(ik, iks, active_index, buf);
        if (icp == TDB_ICP_NO_MATCH)
        {
            if (is_backward)
                tidesdb_iter_prev(scan_iter);
            else
        ...
        if (icp == TDB_ICP_OUT_OF_RANGE) return HA_ERR_END_OF_FILE;
        if (TDB_ICP_ABORTED(icp, cached_thd_)) return HA_ERR_QUERY_INTERRUPTED;

        /* TDB_ICP_MATCH -- condition satisfied (or ICP not applicable) */
        int ret;
...
int ha_tidesdb::index_next(uchar *buf)
{
    ...
    else
    {
        ...
        for (;;)
        {
            ...
            if (icp == TDB_ICP_OUT_OF_RANGE) DBUG_RETURN(HA_ERR_END_OF_FILE);
            if (TDB_ICP_ABORTED(icp, cached_thd_)) DBUG_RETURN(HA_ERR_QUERY_INTERRUPTED);
            ...
            if (ret == HA_ERR_KEY_NOT_FOUND)
            {
                /* The entry names no row this snapshot can see; step over it rather than
                   ending the scan.  index_read_map carries the full reasoning. */
                tidesdb_iter_next(scan_iter);
                continue;
            }
            scan_dir_ = DIR_FORWARD;
...
int ha_tidesdb::index_prev(uchar *buf)
{
    ...
    else
    {
        ...
        for (;;)
        {
            ...
            if (ret == HA_ERR_KEY_NOT_FOUND)
            {
                /* Same as the forward scan: the entry names no row, so step past it. */
                tidesdb_iter_prev(scan_iter);
                continue;
            }
            scan_dir_ = DIR_BACKWARD;
...
int ha_tidesdb::index_last(uchar *buf)
{
    ...
    else
    {
        ...
        const uint idx_key_len = share->idx_comp_key_len[active_index];
        for (;;)
        {
            if (!tidesdb_iter_valid(scan_iter)) DBUG_RETURN(HA_ERR_END_OF_FILE);
            ...
            const int ret =
                fetch_row_by_pk(scan_txn, ik + idx_key_len, (uint)(iks - idx_key_len), buf);
            if (ret != HA_ERR_KEY_NOT_FOUND)
            {
                if (ret == 0) scan_dir_ = DIR_BACKWARD;
                DBUG_RETURN(ret);
            }
            /* The last entry names no row; the one before it may. */
            tidesdb_iter_prev(scan_iter);
        }
    }
}
...
int ha_tidesdb::index_next_same(uchar *buf, const uchar *key [[maybe_unused]], uint keylen [[maybe_unused]])
{
    ...
    for (;;)
    {
        ...
        if (ret == HA_ERR_KEY_NOT_FOUND)
        {
            /* The entry names no row; step over it and keep to the same search prefix. */
            tidesdb_iter_next(scan_iter);
            continue;
        }
        DBUG_RETURN(ret);
...
*/
int ha_tidesdb::multi_range_read_next(TDB_MRR_RANGE_ID *range_info)
{
