                                     ...
                                     bool for_fk_ref)
{
    ...
    for (uint p = 0; p < num_parts && p < key_info->user_defined_key_parts; p++)
    {
        ...
        if (field->type() == MYSQL_TYPE_VARCHAR && field->charset() == &my_charset_bin)
        {
            ...
            uint len_bytes = TDB_VARSTRING_LENGTH_BYTES(fvs);
...
uint comparable_key_length(const KEY *ki)
{
    ...
    {
        if (TDB_FIELD_IS_NULLABLE(ki->key_part[p].field)) len++;
                                          ...
                                          uchar *buf, my_ptrdiff_t ptrdiff)
{
    for (uint p = 0; p < key->user_defined_key_parts; p++)
    {
        ...
        if (TDB_FIELD_IS_NULLABLE(f))
        {
            ...
            {
                f->set_null(ptrdiff);
                /* A null part still occupies its full width in the key: make_comparable_key writes
                   the indicator and then zeroes kp->length value bytes, so every part keeps a fixed
                   span and the parts after it stay at a predictable offset.  Skipping only the
                   indicator here leaves the rest of the key misaligned, which decodes the following
                   column out of the wrong bytes -- silently, and only for a row whose leading part
                   is null. */
                pos++;
...
bool ha_tidesdb::decode_sort_key_part(const uint8_t *src, uint sort_len, Field *f, uchar *buf)
{
    ...
    uchar *to = buf + (uintptr_t)(TDB_FIELD_PTR(f) - f->table->record[0]);
...
/*
  ...
  TDB_ICP_MATCH is returned so the caller falls through to the PK lookup.
*/
    ...
    for (uint p = 0; p < key->user_defined_key_parts; p++)
    {
        ...
        if (TDB_FIELD_IS_NULLABLE(f))
        {
            ...
            if (nb == 0)
            {
                ...
                f->set_null(ptrdiff);
                /* A null part still occupies its full width in the key -- make_comparable_key
                   writes the indicator then zeroes kp->length value bytes -- so the value bytes are
                   skipped here as well.  Advancing past the indicator alone leaves every later part
                   decoded from the wrong offset, which for a pushed condition means matching on
                   another column's bytes and filtering the wrong rows. */
                pos++;
                if (pos + kp->length > end) return false;
                pos += kp->length;
                continue;
    ...
    if (!decode_ok)
    {
        ...
        if (iks > idx_col_len)
        {
            ...
            if (fetch_row_by_pk(scan_txn, pk, pk_len, buf) != 0)
                return TDB_ICP_MATCH; /* PK lookup failed -- we accept row, let caller handle */
        }
        else
        {
            return TDB_ICP_MATCH; /* malformed key -- we accept */
        }
    }

    /* Delegate to the server's ICP evaluator which checks kill state,
       end_range, and pushed_idx_cond->val_bool(). */
    return TDB_INDEX_COND_CHECK(this);
}
