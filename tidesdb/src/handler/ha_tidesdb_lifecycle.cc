...
int ha_tidesdb::rename_table(const char *from, const char *to TDB_DD_RENAME_ARG)
{
...
static void force_remove_cf_dir(const std::string &cf_name)
{
    ...
    /* std::filesystem::remove_all is the portable recursive directory removal
       (handles Windows, symlinks, read-only attrs, etc.). */
    std::error_code rm_ec;
    if (std::filesystem::remove_all(dir, rm_ec) == static_cast<std::uintmax_t>(-1))
...
/*
  Handlerton-level drop_table callback, on servers that publish the hook.  It is called
  instead of handler::delete_table.  Must return 0 on success, not -1.
*/
int tidesdb_hton_drop_table(handlerton *, const char *path)
{
    return tdb_ddl_drop_table_cfs_now(path);
}
...
/*
  Handlerton-level drop_database callback.  the server calls this when the
...
*/
void tidesdb_hton_drop_database(handlerton *, TDB_HTON_DROP_DATABASE_PATH *path)
{
...
int ha_tidesdb::delete_table(const char *name TDB_DD_DELETE_ARG)
{
    DBUG_ENTER("ha_tidesdb::delete_table");
    /* Whether the column families go now or once the statement's dictionary changes are durable is
       the atomicity layer's decision, not this one's. */
    DBUG_RETURN(tdb_ddl_drop_table(ha_thd(), name));
}

/* ******************** delete_all_rows (TRUNCATE) ******************** */

/*
  TRUNCATE TABLE -- empty the table and restart the auto-increment counter.

  Emptying alone is not enough: a truncated table must hand out 1 again on the next insert, and the
  engine keeps that counter in memory on the share, so leaving it alone would make TRUNCATE look
  like a bulk DELETE to the next INSERT.  reset_auto_increment(0) is what restarts it, and it also
  resets the hidden row-id a table without a user primary key is keyed by.
*/
int ha_tidesdb::truncate(TDB_TRUNCATE_ARG)
{
    DBUG_ENTER("ha_tidesdb::truncate");
    const int error = delete_all_rows();
    DBUG_RETURN(error ? error : reset_auto_increment(0));
}
...
int ha_tidesdb::delete_all_rows(void)
{
    ...
    /* A truncated table keeps the storage it was created with -- the options come from the share,
       which holds what was resolved when the table was opened, not from the session as it stands
       now. */
    const ha_table_option_struct *t_opts = &share->options;
    tidesdb_column_family_config_t cfg = build_cf_config(t_opts);
    tidesdb_column_family_config_t data_cfg = data_cf_config(cfg, t_opts->encrypted);

    {
        ...
        /* The recreated family is empty, the record of the table's options among what it lost. */
        TDB_TABLE_OPTIONS_STORE(share->cf, t_opts);
    }
