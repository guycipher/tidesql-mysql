...
int ha_tidesdb::open_init_share_columns(const char *name)
{
    ...
    /* What the table was created with, if the engine recorded it; otherwise what the server
       records plus the session defaults, which is both what a server that resolves the options
       itself always gives and what a table created before they were recorded has. */
    if (!TDB_TABLE_OPTIONS_LOAD(share->cf, &share->options))
    {
        const ha_table_option_struct *named = TDB_TABLE_OPTIONS(table);
        if (named) share->options = *named;
    }

    {
        uint iso_idx = share->options.isolation_level;
        if (iso_idx < array_elements(tdb_isolation_map))
            share->isolation_level = (tidesdb_isolation_level_t)tdb_isolation_map[iso_idx];
    }

    share->default_ttl = share->options.ttl;

    share->encrypted = false;
    share->encryption_key_id = TIDESDB_DEFAULT_ENCRYPTION_KEY_ID;
    share->encryption_key_version = 0;
    if (share->options.encrypted)
    {
        share->encrypted = true;
        share->encryption_key_id = (uint)share->options.encryption_key_id;
        uint ver = tdb_crypto_latest_key_version(share->encryption_key_id);
        if (ver == TDB_CRYPTO_KEY_VERSION_INVALID)
        {
    ...
    for (uint i = 0; i < table->s->fields; i++)
    {
        if (TDB_FIELD_HAS_FLAG(table->s->field[i], BLOB_FLAG))
        {
...
void ha_tidesdb::open_build_field_plan()
{
    ...
    for (uint i = 0; i < table->s->fields; i++)
    {
        ...
        fp.src_off = (uint32)(TDB_FIELD_PTR(f) - table->record[0]);
        fp.pack_len = (uint16)f->pack_length();
        fp.maybe_null = TDB_FIELD_MAYBE_NULL(f);
        fp.is_blob = TDB_FIELD_HAS_FLAG(f, BLOB_FLAG);
        fp.memcpy_ok = false;
        ...
        if (TDB_FIELD_HAS_FLAG(f, BLOB_FLAG)) fp.memcpy_ok = false;
#endif
...
void ha_tidesdb::open_build_index_meta(const char *name)
{
    ...
    for (uint i = 0; i < table->s->keys; i++)
    {
        ...
        tidesdb_column_family_t *icf =
            resolve_idx_cf(tdb_global, share->cf_name, TDB_KEY_NAME(&table->key_info[i]), idx_name);
...
int ha_tidesdb::create(const char *name, TABLE *table_arg,
                       HA_CREATE_INFO *create_info TDB_DD_CREATE_ARG)
{
    DBUG_ENTER("ha_tidesdb::create");

    std::string cf_name = path_to_cf_name(name);

    /* An option this build does not recognise is refused here, naming itself, rather than stored
    ...
    /* Settle the table's storage here, while the statement that asked for it is what decides the
       options, so a later open reads them back rather than resolving the defaults again. */
    TDB_TABLE_OPTIONS_STORE(tidesdb_get_column_family(tdb_global, cf_name.c_str()), opts);
    ...
    for (uint i = 0; i < table_arg->s->keys; i++)
    {
        ...
        std::string idx_cf = cf_name + CF_INDEX_INFIX + TDB_KEY_NAME(&table_arg->key_info[i]);
    ...
    tdb_ddl_note_created(ha_thd(), name);
