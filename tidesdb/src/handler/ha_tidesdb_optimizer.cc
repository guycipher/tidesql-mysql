...
double ha_tidesdb::read_amp()
{
    /* The levels a point lookup expects to touch, sampled with the rest of the table statistics.
       Before the first sample the single-level default keeps a lookup from being priced below the
       one level it must always read. */
    if (!share) return TIDESDB_DEFAULT_READ_AMP;
    return share->cached_read_amp.load(std::memory_order_relaxed);
}

double ha_tidesdb::scan_overlap()
{
    /* The number of sorted runs a full scan would merge.  This is the part of a scan's cost that
       the server's volume estimate cannot see, and it is what both cost adapters add on top.

       tidesdb_range_stats reads in-memory metadata rather than touching disk, but it still costs
       measurable CPU per query plan, so the answer is cached on the share and refreshed on the same
       interval as the rest of the statistics.  The timestamp is stamped on every successful probe,
       a zero-overlap memtable-resident table included, so the refresh gate actually holds instead
       of re-probing the library on every call. */
    if (!share || !share->cf) return 0.0;

    const auto now = std::chrono::steady_clock::now();
    const long long now_us =
        std::chrono::duration_cast<std::chrono::microseconds>(now.time_since_epoch()).count();
    const long long cached_time = share->scan_cost_time.load(std::memory_order_relaxed);
    double overlap = share->cached_scan_cost.load(std::memory_order_relaxed);

    const bool stale = (cached_time == 0) || (now_us - cached_time > TIDESDB_STATS_REFRESH_US);
...
{
    TDB_COLUMN_MAP_SAVED old_map = TDB_USE_ALL_COLUMNS(table, read_set);

    if (min_key && min_key->key)
    {
        KEY *ki = &table->key_info[inx];
        uint kl = TDB_CALC_KEY_LEN(table, inx, min_key->key, min_key->keypart_map);
        if (is_pk)
        {
    ...
    {
        KEY *ki = &table->key_info[inx];
        uint kl = TDB_CALC_KEY_LEN(table, inx, max_key->key, max_key->keypart_map);
        if (is_pk)
        {
            uchar comp[MAX_KEY_LENGTH];
    ...
    TDB_RESTORE_COLUMN_MAP(table, read_set, old_map);
}
...
ha_rows ha_tidesdb::records_in_range(uint inx, TDB_KEY_RANGE *min_key,
                                     TDB_KEY_RANGE *max_key TDB_RECORDS_IN_RANGE_TAIL)
{
    ...
    {
        KEY *ki = &table->key_info[inx];
        uint parts_used = TDB_COUNT_BITS(min_key->keypart_map);
...
ulong ha_tidesdb::index_flags(uint idx, uint part, bool all_parts [[maybe_unused]]) const
{
    ...
    if (table_share && table_share->primary_key != MAX_KEY && idx == table_share->primary_key)
        flags |= TDB_INDEX_FLAGS_CLUSTERED;
    else
    {
