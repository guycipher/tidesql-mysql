                                     ...
                                     std::string &out)
{
    ...
    /* Fail closed if the keyring cannot satisfy the request (missing version,
       ...
       buffer holds uninitialized stack bytes and tdb_crypto_crypt would
       proceed as if the request had succeeded, producing rows nobody can
       decrypt. */
    if (tdb_crypto_get_key(key_id, key_version, key, &klen) != TDB_CRYPTO_STATUS_OK)
    {
        sql_print_error("[TIDESDB] tdb_crypto_get_key failed for key_id=%u version=%u", key_id,
                        key_version);
        out.clear();
        return false;
    }

    /* Fail closed on a spent or unavailable RNG rather than encrypting under whatever the stack
       held.  A repeated IV under one key leaks equality between rows, and a predictable one leaks
       more than that, so there is no safe way to continue here. */
    unsigned char iv[TIDESDB_ENC_IV_LEN];
    if (!tdb_crypto_random_bytes(iv, TIDESDB_ENC_IV_LEN))
    {
        sql_print_error("[TIDESDB] could not obtain %u random bytes for the row IV",
                        TIDESDB_ENC_IV_LEN);
        out.clear();
        return false;
    }
    ...
    unsigned int enc_len = tdb_crypto_encrypted_length(slen, key_id, key_version);
    ...
    int rc = tdb_crypto_crypt((const unsigned char *)plain.data(), slen,
                              (unsigned char *)&out[TIDESDB_ENC_VERSION_LEN + TIDESDB_ENC_IV_LEN],
                              &dlen, key, klen, iv, TIDESDB_ENC_IV_LEN, TDB_CRYPTO_FLAG_ENCRYPT,
                              key_id, key_version);
    if (rc != TDB_CRYPTO_STATUS_OK)
    {
        sql_print_error("[TIDESDB] tdb_crypto_crypt(encrypt) failed rc=%d", rc);
        out.clear();
        return false;
    }
...
static std::string tidesdb_decrypt_row(const char *data, size_t len, uint key_id)
{
    ...
    /* Fail closed if the keyring cannot return the version this row was
       ...
       feed garbage into tdb_crypto_crypt and silently corrupt the
       deserialize path. */
    if (tdb_crypto_get_key(key_id, key_version, key, &klen) != TDB_CRYPTO_STATUS_OK)
    {
        sql_print_error("[TIDESDB] tdb_crypto_get_key failed for key_id=%u version=%u", key_id,
    ...
    int rc = tdb_crypto_crypt(src, slen, (unsigned char *)&out[0], &dlen, key, klen, iv,
                              TIDESDB_ENC_IV_LEN, TDB_CRYPTO_FLAG_DECRYPT, key_id, key_version);
    if (rc != TDB_CRYPTO_STATUS_OK)
    {
        sql_print_error("[TIDESDB] tdb_crypto_crypt(decrypt) failed rc=%d", rc);
        return std::string(); /* signal failure */
    }
    out.resize(dlen);
    return out;
...
/*
  Point a blob-family field at the packed bytes the engine is already holding, the way a server
  whose Field_blob::unpack does not copy would leave it.

  The packed form is the blob's own length prefix followed by its bytes, which is what pack wrote,
  so the length is read back with the field's own accessor rather than assumed.  The field's slot
  lives in the destination record, so writing it goes through move_field_offset when that record is
  not record[0], exactly as the unpack path does.

  Returns the next unread byte, or null when the packed data is short -- a truncated blob is a
  corrupt row, and reading past its end would hand the server a pointer into whatever follows.
*/
static const uchar *tdb_blob_point_at_row(Field *f, uchar *to, const uchar *from,
                                          const uchar *from_end, my_ptrdiff_t ptrdiff)
{
    Field_blob *blob = static_cast<Field_blob *>(f);
    const uint packlength = blob->pack_length_no_ptr();

    if (from + packlength > from_end) return NULL;

    const uint32 length = blob->get_length(from);
    if (from + packlength + length > from_end) return NULL;

    /* set_ptr writes the length and the pointer into the field's slot, which is addressed relative
       to record[0]; shift the field onto the destination record first when they differ. */
    if (ptrdiff != 0) blob->move_field_offset(ptrdiff);
    blob->set_ptr(length, const_cast<uchar *>(from) + packlength);
    if (ptrdiff != 0) blob->move_field_offset(-ptrdiff);

    (void)to;
    return from + packlength + length;
}
...
size_t ha_tidesdb::serialize_estimate_size(const uchar *buf, my_ptrdiff_t ptrdiff)
{
    ...
    if (share->has_blobs)
    {
        ...
        for (uint16 idx : share->blob_field_indices)
        {
            ...
            est += blob->get_length(buf + (uintptr_t)(TDB_FIELD_PTR(f) - table->record[0]));
        }
    }
    return est;
}
...
const std::string &ha_tidesdb::serialize_encrypt_row()
{
    /* We cache the encryption key version per-statement to avoid the
       expensive tdb_crypto_latest_key_version() syscall on every
       single row.  The cache is invalidated at statement start
       (enc_key_ver_valid_ = false in external_lock). */
    if (!enc_key_ver_valid_)
    {
        uint cur_ver = tdb_crypto_latest_key_version(share->encryption_key_id);
        if (cur_ver != TDB_CRYPTO_KEY_VERSION_INVALID)
        {
...
const std::string &ha_tidesdb::serialize_row(const uchar *buf)
{
    ...
    *pos++ = ROW_HEADER_MAGIC;
    ...
    for (uint i = 0; i < nf; i++)
    {
        ...
        {
            pos = TDB_FIELD_PACK(table->field[i], pos, src, UINT_MAX32);
        }
    }
                                           ...
                                           uint unpack_count)
{
    ...
    for (uint i = 0; i < unpack_count; i++)
    {
        ...
        else
        {
            ...
            /* A blob-family column is pointed at the row buffer the engine already holds rather
               than unpacked, where unpacking would copy it into storage the Field shares between
               record[0] and record[1].  The compat layer explains why sharing it silently loses an
               UPDATE that sets only blob columns. */
            if (TDB_FIELD_UNPACK_COPIES_BLOBS && fp.is_blob)
            {
                next = tdb_blob_point_at_row(f, to, from, from_end, ptrdiff);
            }
            else if (ptrdiff != 0)
            {
                f->move_field_offset(ptrdiff);
                next = TDB_FIELD_UNPACK(f, to, from, from_end);
                f->move_field_offset(-ptrdiff);
            }
            else
            {
