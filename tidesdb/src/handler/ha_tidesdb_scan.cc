...
int ha_tidesdb::rnd_init(bool scan [[maybe_unused]])
{
...
int ha_tidesdb::index_init(uint idx, bool sorted [[maybe_unused]])
{
...
/*
  Range scan entry point.  the server hands us both bounds, so we encode them the
